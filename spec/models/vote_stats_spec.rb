...
describe VoteStats do
  describe 'methods' do
    describe 'refresh_percentage' do
      before do
        clear_redis
      end

      it 'returns 0 for each contestant when there are no votes' do
        expect(VoteStats.refresh_percentage).to eq({ percentages: { '1' => 0, '2' => 0 }, greater: '1'}.to_json)
      end
    ...
    describe 'percentage' do
      it 'refreshes the percentage' do
        allow(VoteStats).to receive(:refresh_percentage) { 'refreshed' }
        expect(VoteStats.percentage).to eq 'refreshed'
      end
    end
